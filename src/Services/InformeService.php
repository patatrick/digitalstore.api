...
class InformeService
{
	public function Ventas(int $id_usuario, int $id_tienda, string $where) : InformeDTO | null
	{
		try
		{
			...
			$q = "	SELECT i.nombre_producto nombre, ROUND((d.precio / d.cant), 0) precio_un, d.*,
                    CONVERT(d.mayorista, DECIMAL) mayorista
					FROM detalles d
            ...
            // print_r(json_encode($arrDetalle, JSON_UNESCAPED_UNICODE));
            // die();
