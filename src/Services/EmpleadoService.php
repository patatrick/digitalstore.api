...
class EmpleadoService
{
	...
	public function GetOne(string $cod, int $id_tienda) : Empleado
	{
		try
		{
			$db = MySql::Connect();
			$q = "SELECT * FROM empleados WHERE cod = ? AND id_tienda = ?";
			$stmt = $db->prepare($q);
			...
			$db = null;
			echo "EmpleadoService " . $e->getMessage()." in line ".$e->getLine();
	...
	public function Insert(Empleado $empleado, int $id_tienda) : bool
	{
		try
		{
			...
			$stmt->bindParam(":id_tienda", $empleado->id_tienda, PDO::PARAM_STR);
    ...
    /** Elimina a un empleado de una tienda */
	public function Update(Empleado $empleado, int $id_tienda) : bool
	{
		try
		{
			...
			$q = "	UPDATE empleados SET 
					...
					id_comuna = :id_comuna, 
					telefono = :telefono
					WHERE id_tienda = :id_tienda AND cod = :cod
				";
			$stmt = $db->prepare($q);
			$stmt->bindParam(":ci", $empleado->ci, PDO::PARAM_STR);
			$stmt->bindParam(":id_tienda", $empleado->id_tienda, PDO::PARAM_STR);
